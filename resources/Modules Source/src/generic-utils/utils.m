...
@import ArchiveFile;
#import "utils.h"
@import KernelUtilities;

#define kCFCoreFoundationVersionNumber_iOS_12_0 1535.12 // maybe we do need a common jailbreak header..
...
void injectDir(NSString *dir) {
    ...
    NSLog(@"Will inject %lu files for %@", (unsigned long)toInject.count, dir);
    if (toInject.count > 0) {
        if (injectedToTrustCache) {
            NSLog(@"Warning: Trust cache already injected");
        }
...
bool verifySums(NSString *sumFile, enum hashtype hash) {
    ...
    for (NSString *checksum in [checksums componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]]) {
        ...
        if ([suminfo count] != 2) {
            NSLog(@"Invalid line \"%s\"", checksum.UTF8String);
            return false;
        }
        NSString *fileSum;
        switch (hash) {
        ...
        if (![fileSum.lowercaseString isEqualToString:suminfo[0]]) {
            NSLog(@"Corrupted \"%s\"", [suminfo[1] UTF8String]);
            return false;
        }
        NSLog(@"Verified \"%s\"", [suminfo[1] UTF8String]);
    }
    NSLog(@"No errors in verifying checksums");
    return true;
}
...
bool pkgIsInstalled(char *packageID) {
    ...
    NSLog(@"Deb: \"%s\" is%s installed", packageID, isInstalled?"":" not");
    return isInstalled;
}
...
bool pkgIsConfigured(char *packageID) {
    ...
    NSLog(@"Deb: \"%s\" is%s installed", packageID, isConfigured?"":" not");
    return isConfigured;
}
...
bool pkgIsBy(const char *maintainer, const char *packageID) {
    ...
    NSLog(@"Deb: \"%s\" is%s by %s", packageID, isBy?"":" not", maintainer);
    return isBy;
}
...
bool compareInstalledVersion(const char *packageID, const char *op, const char *version) {
    ...
    rv = !WEXITSTATUS(rv);
    NSLog(@"Deb %s is%s %s %s", packageID, rv?"":" not", op, version);
    return rv;
}

bool runDpkg(NSArray <NSString*> *args, bool forceDeps, bool forceAll) {
    if ([args count] < 2) {
        NSLog(@"%s: Nothing to do", __FUNCTION__);
        return false;
    }
...
bool extractDeb(NSString *debPath, bool doInject) {
    ...
    if (doInject && result) {
        ...
        NSLog(@"Will inject %lu files for %@", (unsigned long)toInject.count, debPath);
        if (toInject.count > 0) {
            if (injectedToTrustCache) {
                NSLog(@"Warning: Trust cache already injected");
            }
...
bool extractDebs(NSArray <NSString *> *debPaths, bool doInject) {
    if ([debPaths count] < 1) {
        NSLog(@"%s: Nothing to install", __FUNCTION__);
        return false;
    }
...
bool removePkgs(NSArray <NSString*> *pkgs, bool forceDeps) {
    if ([pkgs count] < 1) {
        NSLog(@"%s: Nothing to remove", __FUNCTION__);
        return false;
    }
        ...
        if (pkgIsInstalled("org.coolstar.sileo")) {
            ...
            for (NSTextCheckingResult *match in [urlexp matchesInString:cydia_list options:0 range:NSMakeRange(0, cydia_list.length)])
            {
                ...
                if ([url hasPrefix:@"apt.thebigboss.org"] && removeURLFromSources(sileo_sources, @"repounclutter.coolstar.org")) {
                    NSLog(@"Removing duplicated source repounclutter from sileo.sources");
                }
                if (removeURLFromSources(sileo_sources, url)) {
                    NSLog(@"Removing duplicated source %@ from sileo.sources", url);
                }
            }
...
bool daemonIsLoaded(char *daemonID) {
    ...
    NSLog(@"Daemon: \"%s\" is%s loaded", daemonID, isLoaded?"":" not");
    return isLoaded;
}
...
bool modifyPlist(NSString *filename, void (^function)(id)) {
    NSLog(@"%s: Will modify plist: %@", __FUNCTION__, filename);
    NSData *data = [NSData dataWithContentsOfFile:filename];
    if (data == nil) {
        NSLog(@"%s: Failed to read file: %@", __FUNCTION__, filename);
        return false;
    }
    ...
    if (plist == nil) {
        NSLog(@"%s: Failed to generate plist data: %@", __FUNCTION__, error);
        return false;
    }
    if (function) {
        function(plist);
    ...
    if (newData == nil) {
        NSLog(@"%s: Failed to generate new plist data: %@", __FUNCTION__, error);
        return false;
    }
    if (![data isEqual:newData]) {
        NSLog(@"%s: Writing to file: %@", __FUNCTION__, filename);
        if (![newData writeToFile:filename atomically:YES]) {
            NSLog(@"%s: Failed to write to file: %@", __FUNCTION__, filename);
            return false;
        }
    }
...
void list(NSString *directory) {
    ...
    NSLog(@"%s(%@): %@", __FUNCTION__, directory, listArray);
}
...
bool restartSpringBoard() {
    ...
    if (!(backboardd_pid > 1)) {
        NSLog(@"Unable to find backboardd pid.");
        return false;
    }
    if (kill(backboardd_pid, SIGTERM) != ERR_SUCCESS) {
        NSLog(@"Unable to terminate backboardd.");
        return false;
    }
    return true;
}
...
bool verifyECID(NSString *ecid) {
    ...
    if (value == nil) {
        NSLog(@"Unable to get ECID.");
        return false;
    }
    if (![ecid isEqualToString:CFBridgingRelease(value)]) {
        NSLog(@"Unable to verify ECID.");
        return false;
    }
    return true;
}
...
bool installApp(const char *bundle) {
    ...
    if ([applicationWorkspace installApplication:URL withOptions:options]) {
        ...
        NSLog(@"Failed to install application");
        return false;
    }
}
...
bool rebuildApplicationDatabases() {
    ...
    if ([applicationWorkspace _LSPrivateRebuildApplicationDatabasesForSystemApps:YES internal:YES user:NO]) {
        ...
        NSLog(@"Failed to rebuild application databases");
        return false;
    }
}
...
void printOSDetails() {
    ...
    NSLog(@"Machine Name: %s", machineName);
    NSLog(@"Model Name: %s", modelName);
    NSLog(@"Kernel Version: %s", kernelVersion);
    NSLog(@"Kernel Page Size: 0x%lx", (unsigned long) get_kernel_page_size());
    NSLog(@"System Version: iOS %s (%s) (Build: %s)", OSProductVersion, isBetaFirmware() ? "Beta" : "Stable", OSVersion);
out:
    free(machineName);
    free(modelName);
    free(kernelVersion);
...
bool blockDomainWithName(const char *name) {
    if (!unblockDomainWithName(name)) {
        NSLog(@"%s: Unable to clean hosts file", __FUNCTION__);
        return false;
    }
    ...
    if (hosts == nil) {
        NSLog(@"%s: Unable to read hosts file", __FUNCTION__);
        return false;
    }
    ...
    if (![hosts writeToFile:hosts_file atomically:YES encoding:NSUTF8StringEncoding error:nil]) {
        NSLog(@"%s: Unable to update hosts file", __FUNCTION__);
        return false;
    }
    return true;
}
