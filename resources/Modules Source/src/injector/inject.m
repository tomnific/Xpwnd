...
#include <mach/mach.h>
#include <dlfcn.h>
#include <codesign/CSCommon.h>
#ifdef UNDECIMUS
#define INJECT_LOG(x, ...) printf(x, ##__VA_ARGS__)
#define rk64(x) ReadKernel64(x)
#define wk64(x, y) WriteKernel64(x, y)
#else
#define INJECT_LOG(x, ...) printf(x "\n", ##__VA_ARGS__)
#endif
@import Injector.kern_funcs;
...
NSString *cdhashFor(NSString *file) {
    ...
    if (result != errSecSuccess) {
        if (_SecCopyErrorMessageString != NULL) {
            ...
            INJECT_LOG("Unable to generate cdhash for %s: %d", filename, (int)result);
        }
        return nil;
    }
...
NSArray *filteredHashes(uint64_t trust_chain, NSDictionary *hashes) {
  ...
  @autoreleasepool {
    ...
    INJECT_LOG("Actually injecting %lu keys", (unsigned long)[[filtered allKeys] count]);
#if __has_feature(objc_arc)
